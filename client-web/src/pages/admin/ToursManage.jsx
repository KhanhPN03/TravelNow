...
import OriginalTourChart from "./admin_components/ChartOTourCategory";
...
import TourRevenueChart from "./admin_components/ChartTotalRevenue";
import OriginalTourToExcel from "./admin_components/OriginalTourToExcel";
...
function ToursManage() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5000/originalTours/`) // Updated to /originalTours/
      .then((tours) => {
        let fetchedTours = tours.data.originalTours;
        ...
        fetchedTours.forEach((tour) => {
          if (!tour.deleted) {
            tmpTours.push({
              ...
              createdBy:
                tour.createdBy && tour.createdBy.accountCode
  ...
  const handleSoftDelete = async () => {
    try {
      const deletePromises = selectedRows.map((id) =>
        axios.delete(
          `http://localhost:5000/originalTours/softDelete/${id}/${user.user._id}`
        )
      );
  ...
  const handleViewDetails = (id) => {
    navigate(`tourdetail/original/${id}`);
  };

  const handleViewFeedbacks = (id) => {
    navigate(`tourfeedback/original/${id}`);
  };

  const handleExportExcel = async () => {
    setIsExporting(true);
    try {
      await OriginalTourToExcel(filteredData);
      notify("Exported tours successfully", "success");
    } catch (error) {
