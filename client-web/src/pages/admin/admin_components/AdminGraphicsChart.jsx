...
const AdminGraphicsChart = () => {
  ...
  const calculateData = useCallback(
    (data) => {
      ...
      filteredAdmins.forEach((admin) => {
        ...
        // Apply gender filter
        if (selectedGender !== "all" && admin.gender !== selectedGender) {
          return;
        }

        if (admin.gender === "male") genderCount.male++;
        else if (admin.gender === "female") genderCount.female++;
        ...
        if (timePeriod === "day") {
          ...
          key = getWeekNumber(date); // Lấy tuần của ngày
        ...
        if (admin.gender === "male") dateCount[key].male++;
        else if (admin.gender === "female") dateCount[key].female++;
  ...
  useEffect(() => {
    ...
    const fetchAdminData = () => {
      axios
        .get("http://localhost:5000/account/superAdmin/")
        .then((response) => {
          const admins = response.data.admins || [];
          if (admins.length > 0) {
            // Lọc chỉ lấy các admin có deleted = false
            const activeAdmins = admins.filter(
              (admin) => admin.deleted === false
            );
            if (activeAdmins.length > 0) {
              calculateData(activeAdmins);
            } else {
  ...
  const getXAxisLabels = () => {
    if (isTotal) {
      ...
      if (timePeriod === "week") {
        // Nếu chọn "week", hiển thị theo tuần
    ...
    <div className="chartCustomer">
      ...
      <div className="chartCustomer-pieCharts">
        <div className="chartCustomer-barChart">
          <h3>Admin Growth Over Time - {getGenderFilterText()}</h3>
          <Bar
            data={barChartData}
            options={{
              responsive: true,
...
export default AdminGraphicsChart;
