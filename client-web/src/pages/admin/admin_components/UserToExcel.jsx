...
export default async function UserToExcel(customers, type) {
  ...
  // Add headers
  const header = worksheet.addRow(["", "", "", "TravelNow Co.,ltd"]);
  header.height = 30;
  ...
  worksheet.addRow([`Total ${type.toLowerCase()} assigned to ${date} is ${customers.length}`]);
  ...
  saveAs(blob, `${type}_${formatDateForFilename()}.xlsx`);
}